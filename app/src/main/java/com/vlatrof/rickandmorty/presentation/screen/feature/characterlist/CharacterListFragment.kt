...
import com.vlatrof.rickandmorty.databinding.FragmentCharacterListBinding
...
import com.vlatrof.rickandmorty.presentation.screen.feature.characterlist.viewmodel.CharacterListViewModelFactory
...
class CharacterListFragment :
    ...
    private val viewModel: CharacterListViewModel by viewModels { characterListViewModelFactory }
    ...
    private fun setupFilterButton() {
        binding.btnFilter.setDebouncedOnClickListener {
            CharacterListFilterFragment()
                .show(
                    childFragmentManager,
                    ClassUtils.generateTag(CharacterListFilterFragment::class.java)
                )
        }
    }
    ...
    private fun observeFilterState() {
        viewModel.filterState.observe(viewLifecycleOwner) { filter ->
            if (filter.species.isNotBlank() ||
                filter.type.isNotBlank() ||
                filter.gender.isNotBlank() ||
                filter.status.isNotBlank()
            ) {
